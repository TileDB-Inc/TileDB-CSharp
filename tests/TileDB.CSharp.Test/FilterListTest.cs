...
namespace TileDB.CSharp.Test
{
    ...
    public class FilterListTest
    {
        ...
        public void NewFilterListIsValid()
        {
            ...
            positive_delta_filter.set_option(FilterOption.TILEDB_POSITIVE_DELTA_MAX_WINDOW, positiveDeltaMaxWindow);
            filter_list.add_filter(positive_delta_filter);

            const uint maxChunkSize = 512;
            filter_list.set_max_chunk_size(maxChunkSize);

            const uint numFilters = 3;

            Assert.AreEqual(numFilters, filter_list.NFilters());
            Assert.AreEqual(maxChunkSize, filter_list.max_chunk_size());
            ...
            Assert.AreEqual(positiveDeltaMaxWindow, filter2.get_option<uint>(FilterOption.TILEDB_POSITIVE_DELTA_MAX_WINDOW));
        }
    }
